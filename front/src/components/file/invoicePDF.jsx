...
const InvoicePDF = ({ invoice }) => {
  ...
  <Document >
    <Page style={styles.page}>
      <Text style={styles.header}>{t("facture")} N° : {invoice?.number}</Text>
      ...
      <Text style={styles.section}>{t("address")+" Client"}{invoice?.client?.address}</Text>
      ...
      <Text style={styles.section}>{t('montantTotal')+" "}HT: {invoice?.totalHT}</Text>
      <Text style={styles.section}>TVA ({invoice?.taxRate}%): {invoice?.taxAmount}</Text>
      <Text style={styles.section}>{t('montantTotal')+" "}TTC: {invoice?.totalTTC}</Text>
      <Text style={styles.section}> {t("conditionsPaiement")+' '+invoice?.paymentTerms}</Text>
    </Page>
  </Document>
)};
