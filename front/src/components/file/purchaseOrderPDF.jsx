...
const PurchaseOrderPDF = ({ order }) => {
  ...
  <Document>
    <Page style={styles.page}>
      <Text style={styles.header}>{t("bonCommande")} N° : {order?.number}</Text>
      ...
      <Text style={styles.section}>{t("address")+" "+t("fournisseur")} {order?.supplier?.address}</Text>
      <Text style={styles.subHeader}>Client :</Text>
      <Text style={styles.section}>{t("nomClients")+" "+order?.client?.name}</Text>
      <Text style={styles.section}>{t("address")+" Client"}{order?.client?.address}</Text>
      ...
      <Text style={styles.section}>{t('montantTotal')+" "}HT: {order?.totalHT}</Text>
      <Text style={styles.section}>TVA ({order?.taxRate}%): {order?.taxAmount}</Text>
      <Text style={styles.section}>{t('montantTotal')+" "}TTC: {order?.totalTTC}</Text>
      <Text style={styles.section}> {t("conditionsPaiement")+' '+order?.paymentTerms}</Text>
    </Page>
  </Document>
)};
